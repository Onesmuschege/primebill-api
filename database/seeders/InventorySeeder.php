...
class InventorySeeder extends Seeder
{
    ...
    public function run(): void
    {
        $this->forEachTenant(function (Tenant $tenant) {
            ...
            $items = [
                ['name' => 'MikroTik hAP ac2', 'category' => 'Routers', 'quantity' => 8, 'unit_cost' => 4500.00, 'serial_number' => 'MTK-HAP-001', 'status' => 'in_stock', 'low_stock_alert' => 3, 'client' => null],
                ...
                ['name' => 'MikroTik hAP ac2 (Assigned)', 'category' => 'Routers', 'quantity' => 1, 'unit_cost' => 4500.00, 'serial_number' => 'MTK-HAP-021', 'status' => 'assigned', 'low_stock_alert' => 1, 'client' => 0],
            ...
            foreach ($items as $item) {
                $clientId = null;
                if ($item['client'] !== null && $clients->isNotEmpty()) {
                    $clientId = $clients[$item['client'] % $clients->count()]->id;
                    $assignedCount++;
                }
            ...
            $this->command->line("  [{$tenant->slug}] " . count($items) . " inventory items seeded (" . $assignedCount . " assigned to clients).");
        });

        $this->command->info('InventorySeeder: complete.');
    }
}